...
| 7b | AUTH MATRIX | ✅ shipped | `broken_auth` / `privilege_escalation` prove here (matrix-driven, not single-probe) gated on steered context — same pure `security_graph` judges on a fresh graph, adapted via the single `VALIDATED→CONFIRMED` site; broken_auth captures its token **independently** from credentials (headless HTTP form login, no external driver) and honours a `forge_claims` escalation target; a CONFIRMED forgery may then **demonstrate its impact** — dynamically discover the declared privileged action off the live admin page the bypass unlocked and exercise it with the forged token (doubly gated: check declares `impact` **and** `SENTINEL_ENABLE_IMPACT=1`; itself a forged-vs-anonymous differential; state-changing, so never default); no token/password in any note |
